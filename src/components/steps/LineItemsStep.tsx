...
import { ShoppingCart, Plus, Trash2, Calculator, Edit, RotateCcw } from "lucide-react";
...
export const LineItemsStep = ({ data, taxRate, onChange, onTaxRateChange }: LineItemsStepProps) => {
  ...
  const addPredefinedService = (service: PredefinedService) => {
    const newItem: LineItem = {
      id: `item-${Date.now()}`,
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in fade-in duration-500 delay-400">
            ...
            <CardContent className="space-y-4">
              {data.length === 0 ? (
                ...
                <div className="space-y-4">
                  {data.map((item, index) => (
                    <div key={item.id} className="bg-white/60 p-4 rounded-lg border border-primary/10 transition-all duration-300 hover:bg-white/80 hover:shadow-md animate-in slide-in-from-left-3 duration-300" style={{ animationDelay: `${index * 100}ms` }}>
                      <div className="flex items-center justify-between mb-3">
                        <Badge variant="secondary" className="bg-primary/10 text-primary animate-in zoom-in duration-200" style={{ animationDelay: `${index * 100 + 50}ms` }}>
                          Service #{index + 1}
                        </Badge>
                        <Button
                          variant="ghost"
              ...
              <Button onClick={addLineItem} className="w-full transition-all duration-300 hover:shadow-lg hover:scale-[1.02]" variant="outline">
          ...
          <Card className="shadow-xl border-0 backdrop-blur-sm transition-all duration-300 hover:shadow-2xl" style={{ background: 'var(--gradient-card)' }}>
            ...
            <CardContent className="space-y-2 max-h-96 overflow-y-auto">
              {services.map((service, index) => (
                <div
                  key={service.id}
                  className="flex items-center justify-between p-3 rounded-lg bg-white/40 hover:bg-white/60 transition-all duration-200 animate-in slide-in-from-right-2"
                  style={{ animationDelay: `${index * 50}ms` }}
                >
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-sm truncate">{service.description}</div>
                  ...
                  <div className="flex items-center gap-2 ml-3">
                    <Button
                      ...
                      className="h-8 w-8 p-0 text-primary hover:bg-primary/10 transition-all duration-200"
                    >
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="h-10 border-0 bg-white shadow-sm transition-all duration-200 focus:shadow-md"
                />
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between animate-in fade-in duration-300 delay-100">
                  <span className="text-muted-foreground">Tax ({(taxRate * 100).toFixed(1)}%):</span>
                  <span className="font-semibold">${tax.toFixed(2)}</span>
                </div>
                <Separator />
                <div className="flex justify-between text-lg animate-in fade-in duration-300 delay-200">
